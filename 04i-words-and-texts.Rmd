...
If you do not learn how to edit and transform text-based fields within datasets, you will quickly get stuck in `R`. Think of dates, GPS coordinates, user IDs, group names, plot labels, etc. All of these forms of data can contain non-numeric text. Becoming comfortable working with text in `R` is an essential part of your `R` toolkit.
...
**`paste()` & `paste0()`**
`paste()` and `paste0()` combines two or more strings together into a single object:
...
Make an object called `ages` that has the age of everyone in your group and use `paste` and `collapse` to list these ages.

**`tolower()` & `toupper()`**
`tolower()` and `toupper()` forces all text in a string to lower case or upper case, respectively. Run the following:
...
Instead of `/` replace put `.` between the year, month, day.
